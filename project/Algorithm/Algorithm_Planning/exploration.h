...
struct ObsPoint {
    ...
    uint8_t entity_id;   // 属于哪个实体？ [0, MAX_ENTITIES)
...
private:
    StaticArray<ObsPoint, MAX_OBS_POINTS> obs_points;    // 所有合法的观测点列表
    uint8_t total_entities;                  // 观测点对应的实体总数（箱子+目标点）
    SokobanLevel cached_level;               // 内部缓存的绝对真理地图

    float cost_matrix[MAX_OBS_POINTS][MAX_OBS_POINTS];   // 任意两个观测点之间的代价矩阵 (物理距离 + 朝向差的加权和)
